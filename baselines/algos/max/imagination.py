...
class Imagination:
    def __init__(self, model, n_actors, horizon, measure):
        ...
        self.action_space = Box(
            low=-1.0, high=1.0, shape=(n_actors, self.model.d_action), dtype=np.float32
        )
    ...
    def step(self, actions):
        ...
        with torch.no_grad():
            next_state_means, next_state_vars = self.model.forward_all(
                self.states, actions
            )  # shape: (n_actors, ensemble_size, d_state)

        i = torch.arange(n_act).to(self.model.device)
        j = torch.randint(es, size=(n_act,)).to(self.model.device)
        next_states = self.model.sample(
            next_state_means[i, j], next_state_vars[i, j]
        )  # shape: (n_actors, d_state)
        ...
        measures = self.measure(
            self.states,  # shape: (n_actors, d_state)
