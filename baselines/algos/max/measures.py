...
class Measure:
    def __call__(
        self, states, actions, next_states, next_state_means, next_state_vars, model
    ):
        """
...
class UtilityMeasure(Measure):
    ...
    def __call__(
        ...
        utility = self.compute_utility(
            states, actions, next_states, next_state_means, next_state_vars, model
        )
        ...
        if not np.allclose(self.action_norm_penalty, 0):
            ...
            utility = (
                utility - self.action_norm_penalty * action_norms
            )  # shape: (n_actors)
...
class SlowJensenRenyiDivergenceUtilityMeasure(UtilityMeasure):
    def compute_utility(
        ...
        total_entropy = torch.log(
            ((2 * np.pi) ** d_s) * total_entropy
...
class JensenRenyiDivergenceUtilityMeasure(UtilityMeasure):
    ...
    def rescale_var(self, var, min_log_var, max_log_var):
        _, max_var = np.exp(min_log_var), np.exp(max_log_var)
        return max_var - self.decay * (max_var - var)

    def compute_utility(
        self, states, actions, next_states, next_state_means, next_state_vars, model
    ):
        state_delta_means = next_state_means - states.to(
            next_state_means.device
        ).unsqueeze(1)
        state_delta_means = model.normalizer.renormalize_state_delta_means(
            state_delta_means
        )
        state_delta_vars = model.normalizer.renormalize_state_delta_vars(
            next_state_vars
        )
        ...
        # entropy of the mean
        mu_diff = mu.unsqueeze(1) - mu.unsqueeze(
            2
        )  # shape: (n_actors, ensemble_size, ensemble_size, d_state)
        var_sum = var.unsqueeze(1) + var.unsqueeze(
            2
        )  # shape: (n_actors, ensemble_size, ensemble_size, d_state)

        err = (
            mu_diff * 1 / var_sum * mu_diff
        )  # shape: (n_actors, ensemble_size, ensemble_size, d_state)
        err = torch.sum(err, dim=-1)  # shape: (n_actors, ensemble_size, ensemble_size)
        det = torch.sum(
            torch.log(var_sum), dim=-1
        ...
        # mean of entropies
        total_entropy = torch.sum(
            torch.log(var), dim=-1
        )  # shape: (n_actors, ensemble_size)
        mean_entropy = (
            total_entropy.mean(dim=1) / 2 + d_s * np.log(2) / 2
        )  # shape: (n_actors)
