...
class ActionValueFunction(nn.Module):
    def __init__(self, d_state, d_action, n_hidden):
        ...
        self.layers = nn.Sequential(
            ParallelLinear(d_state + d_action, n_hidden, ensemble_size=2),
            nn.LeakyReLU(),
...
class StateValueFunction(nn.Module):
    def __init__(self, d_state, n_hidden):
        ...
        self.layers = nn.Sequential(one, nn.LeakyReLU(), two, nn.LeakyReLU(), three)
...
class GaussianPolicy(nn.Module):
    ...
    def forward(self, state):
        ...
        pi = torch.tanh(pi)
        logp_pi -= torch.sum(
            torch.log(torch.clamp(1 - pi.pow(2), min=0, max=1) + EPS),
            dim=1,
            keepdim=True,
        )
...
class SAC(nn.Module):
    ...
    def update(self):
        ...
        states, actions, rewards, next_states, masks = [
            s.to(self.device) for s in sample
        ]
    ...
    def episode(self, env, warm_up=False, train=True, verbosity=0, _log=None):
        ...
        while not done:
            ...
            if verbosity >= 3 and _log is not None:
                _log.info(
                    f"step_reward. mean: {torch.mean(rewards).item():5.2f} +- {torch.std(rewards).item():.2f} [{torch.min(rewards).item():5.2f}, {torch.max(rewards).item():5.2f}]"
                )
